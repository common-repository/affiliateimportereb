    ...
    class EBDN_Goods
    {
        ...
        public static function getNormalizedValue($goods, $field)
        {
            ...
            if (property_exists(get_class($goods), $field)) {
                $result = $goods->$field;
                $result = EBDN_Goods::normalized($result);
            }
            return $result;
        }
        ...
        public function save($action = 'ALL')
        {
            ...
            if ($this->loaded) {
                $wpdb->update($wpdb->prefix . EBDN_TABLE_GOODS, $data, ['type' => $this->type, 'external_id' => $this->external_id, 'variation_id' => $this->variation_id]);
            } else {
                $wpdb->insert($wpdb->prefix . EBDN_TABLE_GOODS, $data);
            }
        ...
        public function saveField($field, $value = "")
        {
            ...
            if ($this->loaded) {
                ...
                $wpdb->update(
                    $wpdb->prefix . EBDN_TABLE_GOODS_ARCHIVE,
                    $data,
                    [
                        'type' => $this->type,
                        'external_id' => $this->external_id,
        ...
        public function load()
        {
            ...
            $this->loaded = false;
            if ($this->type && $this->external_id) {
                $results = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . EBDN_TABLE_GOODS . " WHERE type='$this->type' and external_id='$this->external_id' and variation_id='$this->variation_id'");
                if ($results) {
        ...
        public static function loadGoodsList($page, $per_page, $filter = "")
        {
            ...
            $db_res = $wpdb->get_results('SELECT '.$wpdb->prefix . EBDN_TABLE_GOODS.'.* FROM ' . $wpdb->prefix . EBDN_TABLE_GOODS .
                ' left join ' . $wpdb->prefix . EBDN_TABLE_BLACKLIST . ' on 
                ' . $wpdb->prefix . EBDN_TABLE_GOODS . '.external_id = ' . $wpdb->prefix . EBDN_TABLE_BLACKLIST . '.external_id ' .
                $where . ' AND ' . $wpdb->prefix . EBDN_TABLE_BLACKLIST . '.id is null ' .
                ' LIMIT ' . (($page - 1) * $per_page) . ', ' . $per_page);
            if ($db_res) {
                foreach ($db_res as $row) {
                    $result[] = new EBDN_Goods($row, true);
                }
            }
        ...
        public static function clearList($delete_schedule_post = false)
        {
            ...
            if ($delete_schedule_post) {
                $wpdb->query('TRUNCATE ' . $wpdb->prefix . EBDN_TABLE_GOODS);
            } else {
                $wpdb->query('DELETE FROM ' . $wpdb->prefix . EBDN_TABLE_GOODS . " WHERE NULLIF(NULLIF(user_schedule_time, '0000 - 00 - 00 00:00:00'), '') IS NULL");
            }
        }
...
if (!function_exists('ebdn_sort_by_price')) {

    function ebdn_sort_by_price($g1, $g2)
    {
        $a = (float)$g1->price;
        $b = (float)$g2->price;
        if (abs($a - $b) < 0.001) {
...
if (!function_exists('ebdn_sort_by_user_price')) {

    function ebdn_sort_by_user_price($g1, $g2)
    {
...
if (!function_exists('ebdn_sort_by_ship')) {

    function ebdn_sort_by_ship($g1, $g2)
    {
        $a = (isset($g1->additional_meta['ship']) && EBDN_Goods::normalized($g1->additional_meta['ship'])) ? (float)$g1->additional_meta['ship'] : 0.00;
        $b = (isset($g2->additional_meta['ship']) && EBDN_Goods::normalized($g2->additional_meta['ship'])) ? (float)$g2->additional_meta['ship'] : 0.00;
        if (abs($a - $b) < 0.001) {
            return 0;
        }
        return ($a < $b) ? -1 : 1;
...
if (!function_exists('ebdn_sort_by_ship_to_locations')) {

    function ebdn_sort_by_ship_to_locations($g1, $g2)
    {
        $a = (isset($g1->additional_meta['ship_to_locations']) && EBDN_Goods::normalized($g1->additional_meta['ship_to_locations'])) ? $g1->additional_meta['ship_to_locations'] : '0.00';
        $b = (isset($g2->additional_meta['ship_to_locations']) && EBDN_Goods::normalized($g2->additional_meta['ship_to_locations'])) ? $g2->additional_meta['ship_to_locations'] : '';
        return strcasecmp($a, $b);
    }

}

if (!function_exists('ebdn_sort_by_curr')) {

    function ebdn_sort_by_curr($g1, $g2)
    {
