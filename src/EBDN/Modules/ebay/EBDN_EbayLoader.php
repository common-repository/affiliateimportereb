...
/**
 * Description of EBDN_EbayLoader
 *
 */
include_once(dirname(__FILE__) . '/EBDN_EbaySession.php');

if (!class_exists('EBDN_EbayLoader')):

    class EBDN_EbayLoader extends EBDN_AbstractLoader
    {

        public function loadList($filter, $page = 1)
        {
            $per_page = get_option('ebdn_ebay_per_page', 20);
            $result = array("total" => 0, "per_page" => $per_page, "items" => array(), "error" => "");

            if ((isset($filter['ebdn_productId']) && !empty($filter['ebdn_productId'])) || (isset($filter['ebdn_query']) && !empty($filter['ebdn_query'])) || (isset($filter['store']) && !empty($filter['store'])) || (isset($filter['category_id']) && $filter['category_id'] != 0)) {
                ...
                $price_min = (isset($filter['ebdn_min_price']) && floatval($filter['ebdn_min_price']) > 0.009) ? floatval($filter['ebdn_min_price']) : 0;
                $price_max = (isset($filter['ebdn_max_price']) && floatval($filter['ebdn_max_price']) > 0.009) ? floatval($filter['ebdn_max_price']) : 0;
                ...
                if ($productId) {
                    $tmp_res = $this->loadDetail(new EBDN_Goods("ebay#$productId"), array("init_load" => true, "link_category_id" => $link_category_id, "site_code" => $site));
                    if ($tmp_res['state'] == 'ok') {
                        $result["total"] = 1;
                    ...
                    $tmp_response = ebdn_remote_get($apicall);
                    if (is_wp_error($tmp_response)) {
                        ...
                        if (isset($resp->errorMessage->error)) {
                            ...
                            if ($resp && $resp->paginationOutput->totalEntries > 0) {
                                ...
                                $currency_conversion_factor = floatval(str_replace(",", ".", strval(get_option('ebdn_currency_conversion_factor', 1))));
                                $tmp_variation_cnt = array();
                                foreach ($resp->searchResult->item as $item) {
                                    //echo "<pre>";print_r($item);echo "</pre>";

                                    $goods = new EBDN_Goods();
                                    $goods->type = "ebay";
                                    ...
                                    $goods->image = ($item->galleryURL) ? strval($item->galleryURL) : EBDN_NO_IMAGE_URL;
                                    ...
                                    $goods->additional_meta['ship'] = EBDN_Goods::getNormalizePrice($item->shippingInfo->shippingServiceCost);
                                    ...
                                    $goods->price = round(EBDN_Goods::getNormalizePrice($item->sellingStatus->convertedCurrentPrice), 2);
                                    ...
                                    if (get_option('ebdn_ebay_using_woocommerce_currency', false) && get_woocommerce_currency() === trim(strval($item->sellingStatus->currentPrice['currencyId']))) {
                                        $goods->price = round(EBDN_Goods::getNormalizePrice($item->sellingStatus->currentPrice), 2);
                                    ...
                                    $result["items"][] = apply_filters('ebdn_modify_goods_data', $goods, $item, "ebay_load_list");
                                }
                            }
                        }
                    }
        ...
        public function loadDetail(&$goods, $params = array())
        {
            ...
            if ($init_load) {
                if ($this->account->use_affiliate_urls()) {
                    if (get_option('ebdn_ebay_custom_id')) {
                        $api_url .= "&trackingid=" . get_option('ebdn_ebay_custom_id');
                    }
                    if (get_option('ebdn_ebay_network_id')) {
                        $api_url .= "&trackingpartnercode=" . get_option('ebdn_ebay_network_id');
                    }
                    if (get_option('ebdn_ebay_tracking_id')) {
                        $api_url .= "&affiliateuserid=" . get_option('ebdn_ebay_tracking_id');
                    }
                }
            }
            ...
            $tmp_response = ebdn_remote_get($api_url);
            if (is_wp_error($tmp_response)) {
                ...
                if (!isset($detail_xml->Errors)) {
                    if ($init_load) {
                        ...
                        $goods->image = ($detail_xml->Item->GalleryURL) ? strval($detail_xml->Item->GalleryURL) : EBDN_NO_IMAGE_URL;
                        ...
                        if (get_option('ebdn_ebay_using_woocommerce_currency', false) && get_woocommerce_currency() === trim(strval($detail_xml->Item->CurrentPrice['currencyID']))) {
                            $goods->price = round(EBDN_Goods::getNormalizePrice($detail_xml->Item->CurrentPrice), 2);
                            $goods->curr = trim(strval($detail_xml->Item->CurrentPrice['currencyID']));
                        } else {
                            $goods->price = round(EBDN_Goods::getNormalizePrice($detail_xml->Item->ConvertedCurrentPrice), 2);
                    ...
                    $goods->description = EBDN_Utils::removeTags($goods->description);
                    ...
                    if ($new_prew && (strlen(trim(strval($goods->user_image))) == 0 || trim(strval($goods->user_image)) === trim(strval($goods->image)))) {
                        if ($goods->image === EBDN_NO_IMAGE_URL) {
                            $goods->image = $new_prew;
                        }
                        $goods->saveField("user_image", $new_prew);
                    }

                    $result = array("state" => "ok", "message" => "", "goods" => apply_filters('ebdn_modify_goods_data', $goods, $detail_xml, "ebay_load_detail"));
                } else {
        ...
        public function checkAvailability(/* @var $goods EBDN_Goods */
            $goods)
        {
            $site_id = 0;
        ...
        public function getDetail($productId, $params = array())
        {
            ...
            if (is_wp_error($tmp_response)) {
                ...
                if (!isset($detail_xml->Errors)) {
                    $goods = new EBDN_Goods("ebay#" . $detail_xml->Item->ItemID);

                    // check_availability
                    $end_time = isset($detail_xml->Item->EndTime) ? strtotime($detail_xml->Item->EndTime) : (time() + 60);
                    $goods->availability = $end_time > time();

                    $goods->image = ($detail_xml->Item->GalleryURL) ? $detail_xml->Item->GalleryURL : EBDN_NO_IMAGE_URL;
                    $goods->detail_url = $detail_xml->Item->ViewItemURLForNaturalSearch;
                    $goods->seller_url = "http://www.ebay.com/usr/" . $detail_xml->Item->Seller->UserID;
                    ...
                    $goods->additional_meta['ship'] = EBDN_Goods::getNormalizePrice($detail_xml->Item->ShippingCostSummary->ShippingServiceCost);
                    ...
                    $result = array("state" => "ok", "message" => "", "goods" => apply_filters('ebdn_modify_goods_data', $goods, $detail_xml, "ebay_get_detail"));
                } else {
