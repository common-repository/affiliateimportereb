...
class Status
{
    public function render()
    {
        ...
        include EBDN_ROOT_PATH . '/layout/status.php';
    }
    ...
    public function checkRemoteGet()
    {
        ...
        $response = wp_remote_get('http://gw.api.Ebay.com/', $params);

        if (!is_wp_error($response) && $response['response']['code'] >= 200 && $response['response']['code'] < 300) {
            $msg = 'wp_remote_get() was successful - Webservices Ebay is working.';
            $status = true;
        } elseif (is_wp_error($response)) {
            $msg = 'wp_remote_get() failed. Webservices Ebay won\'t work with your server. Contact your hosting provider. Error:' . ' ' . $response->get_error_message();
            $status = false;
        } else {
            $msg = 'wp_remote_get() failed. Webservices Ebay may not work with your server.';
            $status = false;
        }
